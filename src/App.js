...
export default function App(){
    ...
    const newNote = () => {
        ...
        let actualDate = currentDate.getFullYear()+"-"+month+"-"+currentDate.getDay()+1;
            ...
            <nav className="navbar navbar-expand-lg navbar-dark bg-primary">
                <div className="container-fluid">
                    ...
                    <div className="collapse navbar-collapse" id="navbarColor01">
                        ...
                        <button className="btn btn-warning text-dark" onClick={newNote}><i className="bi bi-clipboard-plus"></i> Create new Note</button>
                    </div>
                </div>
            </nav>
            {/* Actual App */}
            ...
            <div className='container mt-5'>
                <div className='row' id='noteContainer'>
                    {   
                         notes.map((note) => {
                             return(
                                 <Note key={note.key} title={note.title} text={note.text} due={note.due} id={note.key} />
                             )
                         })
                    }
                </div>
